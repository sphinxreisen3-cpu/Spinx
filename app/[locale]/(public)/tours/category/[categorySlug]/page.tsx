...
import { getCategorySeoBySlug } from '@/config/categories';
...
async function getToursByCategory(categoryName: string, locale: string) {
  const baseUrl = process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000';
  const res = await fetch(
    `${baseUrl.replace(/\/$/, '')}/api/tours?category=${encodeURIComponent(categoryName)}&limit=50&isActive=true`,
    { next: { revalidate: 60 } }
  );
  if (!res.ok) return [];
...
export default async function CategoryLandingPage({
  ...
  const isDe = locale === 'de';
  const name = isDe && cat.nameDe ? cat.nameDe : cat.name;
  const intro = isDe && cat.introDe ? cat.introDe : cat.intro;
  const tours = await getToursByCategory(cat.name, locale);
      ...
      <section className={styles.hero}>
        <div className={styles.heroContent}>
          ...
          <p className={styles.subtitle}>
            {intro ||
              (isDe
                ? `Entdecken Sie unsere ${name}-Touren. Buchen Sie Erlebnisse in Ägypten.`
                : `Discover our ${name} tours. Book experiences in Egypt.`)}
          </p>
      ...
      <section id="tours" className={styles.gridSection}>
        ...
        {tours.length === 0 ? (
          <p style={{ textAlign: 'center', padding: '2rem' }}>
            {isDe ? 'Keine Touren in dieser Kategorie.' : 'No tours in this category.'}
          </p>
        ) : (
